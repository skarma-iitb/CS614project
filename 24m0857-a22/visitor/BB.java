...
public class BB {
    ...
    public static void printBBToStreamAnalysisResult(BB startNode, StringBuilder sb, HashMap<Node, String> resultMap) {
        ...
        while (worklist.size() > 0) {
            ...
            if (currentBB.outgoingEdges.size() == 0) {
                sb.append("\n" + currentBB.name
                        + " [style=\"rounded,filled\", shape=\"box\", fillcolor=\"orange\", fontname=\"monospace\", xlabel=\"End\", label=\"");
            } else if (currentBB.instructions.size() == 1) {
                sb.append("\n" + currentBB.name
                        + " [fillcolor=\"gray\", style=\"filled\", shape=\"doublecircle\", fontname=\"monospace\", label=\"");
            } else if (currentBB.instructions.size() == 2
                    && currentBB.instructions.get(1).instructionNode instanceof Identifier) {
                sb.append("\n" + currentBB.name
                        + " [fillcolor=\"gray\", style=\"rounded,filled\", shape=\"diamond\", fontname=\"monospace\", label=\"");
            } else {
                sb.append("\n" + currentBB.name
                        + " [fillcolor=\"white\", style=\"filled\", shape=\"box\", fontname=\"monospace\", xlabel=\""
                        + currentBB.name + "\", label=\"");
            }
