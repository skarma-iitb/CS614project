...
public class Main {
   public static void main(String[] args) {
      try {
         ...
         ArrayList<String> Methodsinline = tempo2.getMethodsinline();
         Inliningvisitor3 tempo3 = new Inliningvisitor3(possibleMethods);
         HashMap<String, String> varvalue = tempo3.getvarvalue();
         if (possibleMethods.size() >= 1) {
            ...
            StringBuilder il = tempo3.getInlining();
            StringBuilder il2 = tempo3.getInlining2();

            newoutput np = new newoutput(outMap, inMap,
                  programCFG, methodtable, methodtable1, methodvalue, possibleMethods, il, il2, Methodsinline);
            root.accept(np);
            String output1 = np.getOutput1();
            StringReader newinput = new StringReader(output1);
            a2java.ReInit(newinput);
            Node newRoot = a2java.Goal();
            CFGGen cfgGen1 = new CFGGen();
            newRoot.accept(cfgGen1);
            ProgramCFG programCFG1 = cfgGen.getCFG();
            newworklist analysis1 = new newworklist();
            analysis1.analyze(programCFG1);
            Map<BB, Map<String, LatticeValue>> inMap1 = analysis.getInMap();
            Map<BB, Map<String, LatticeValue>> outMap1 = analysis.getOutMap();
            ConstantPropagationVisitor cp = new ConstantPropagationVisitor(outMap1,
                  inMap1,
                  programCFG1, varvalue);
            newRoot.accept(cp);
            // cp.printvar();
            System.out.println(cp.getOutput());
            // System.out.println(np.getOutput1());
         } else {
            Map<BB, Map<String, LatticeValue>> inMap = analysis.getInMap();
            Map<BB, Map<String, LatticeValue>> outMap = analysis.getOutMap();
